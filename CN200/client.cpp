...
string get_file_from_server(int sockfd, struct sockaddr *sock_addr, string out_file_name, uint32_t file_num){
    int port;
    seq_num = client_three_way_handshake(sockfd, sock_addr, &port);
    cumulative_ack = 1;
    opt = file_num;
    ...
    serverInfo.sin_port = htons(port); // use new port from server
    ...
    tcp_header_t send_header = {0};
    printf("file_num : %u\n", file_num);
    send_header.seq_num = (uint32_t)seq_num;
    send_header.ack_num = (uint32_t)cumulative_ack;
    send_header.win = (uint16_t)rwnd;
    send_header.opt = (uint32_t)opt;

    send_tcp_package(sockfd, (struct sockaddr *)&serverInfo, send_header, NULL, 0);


    /* lock until all file be getten */
    recv_exit_mutex.lock();
    send_exit_mutex.lock();
    thread recv0(recv_package_thread, sockfd, (struct sockaddr *)&serverInfo, out_file_name);
    thread send0(send_package_thread, sockfd, (struct sockaddr *)&serverInfo);

    string str;

    // wait for recv and send all data
    recv_exit_mutex.lock();
    recv_exit_mutex.unlock();
...
int main(int argc, char *argv[]){
    ...
    if(argc == 3){
        ...
        out_file_name += "_" + to_string(rand()%10000) + ".mp4";
    }
    else if(argc == 4){

...
mutex recv_exit_mutex;
mutex send_exit_mutex;
    ...
    while(1){
        ...
        if(_file_str == NULL){
            ...
            case 4:
                ...
                break;
            default:
                printf("recv_header.opt = %u\n", recv_header.opt);
                continue; // wait for next package
...
void new_server(int port){
    ...
    // child
        ...
        serverInfo.sin_port = htons(port); // use new port
        ...
        recv_exit_mutex.lock();
        send_exit_mutex.lock();
        printf("new process for client\n");
        //struct sockaddr_in client_addr;
        thread recv0(recv_package_thread, sockfd, (struct sockaddr *)&serverInfo);
        thread send0(send_package_thread, sockfd, (struct sockaddr *)&serverInfo);
...
int main(int argc, char *argv[]){
    ...
    while(1){
        /* find port can use */
        int try_sockfd;
        struct sockaddr_in try_serverInfo = {0};

        /* write server information */
        try_serverInfo.sin_family = AF_INET;
        try_serverInfo.sin_addr.s_addr = htonl(INADDR_ANY); // all ip address can connect
        try_serverInfo.sin_port = htons(_port); // use port 10101
        /* create socket */
        if((try_sockfd= socket(AF_INET, SOCK_DGRAM, 0)) < 0){
            _port++;
            continue;
        }
        /* bind to computer */
        while((bind(try_sockfd, (struct sockaddr *)&try_serverInfo, sizeof(serverInfo))) == -1){
            try_serverInfo.sin_port = htons(++_port); // use new port
            //error_msg("bind socket");
        }
        close(try_sockfd);
        /* find port can use END */

        
        struct sockaddr_in client_addr;
        server_three_way_handshake(sockfd, (struct sockaddr *)&client_addr, _port);
        new_server(_port);
    }

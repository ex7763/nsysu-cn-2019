...
mutex server_handshake_mutex;
mutex client_handshake_mutex;

#define DEBUG_SEND

int client_three_way_handshake(int sockfd, struct sockaddr *socket, int *port){
    lock_guard<std::mutex> lg(client_handshake_mutex);
    uint8_t buf[1024] = {0};
    ...
    /* recv ack/syn */
    /* record port */
    int data_len = recv_tcp_package(sockfd, socket, &recv_header, buf, 1024);
    buf[data_len] = '\0';
    sscanf((char *)buf, "%d", port);
...
int server_three_way_handshake(int sockfd, struct sockaddr *socket, int port){
    lock_guard<std::mutex> lg(server_handshake_mutex);
    ...
    if(recv_header.syn == 1){
        ...
        // send_tcp_package(sockfd, socket, send_header, NULL, 0);
        // send port data
        send_tcp_package(sockfd, socket, send_header, buf, strlen((char *)buf));
    }
...
int send_tcp_package(int sockfd, struct sockaddr *socket, tcp_header_t header, uint8_t *data, uint32_t data_size){
    ...
    #ifdef SEND_PACKAGE_INFORM
    printf("Send package(seq_num = %u, ack_num = %u) data size: %u bytes\n", header.seq_num, header.ack_num, data_size);
    #endif
    return 0;
}

uint32_t recv_tcp_package(int sockfd, struct sockaddr *socket, tcp_header_t *header, uint8_t *buf, uint32_t buf_size){
    /* wait client data */
    socklen_t len = sizeof(*socket);
